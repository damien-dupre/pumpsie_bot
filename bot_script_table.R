...
df <- df |> 
  ...
  left_join(df |> filter(dateupdated == Sys.Date() - 1) |> group_by(county, fuel) |> slice_min(price, with_ties = FALSE) |> select(county, fuel, price_min = price, name, town)) |> 
  ungroup()
...
df |>
  ...
  select(Fuel = fuel, county, N = n, `Avg Price`, Arrow = perc_change, Change = perc_change, Location = name, Zone = town, `Min Price`) |>
  gt(groupname_col = "Fuel") |>
  sub_missing() |>
  opt_row_striping() |> 
  cols_label(Arrow = "", Change = "") |>
  tab_header(
    title = md("**Fuel prices reported on *pickapump.com* yesterday by Irish county**"),
  ...
  tab_footnote(
    footnote = md("Missing counties among the 26 (IE) have no fuel price reported on *pickapump.com* yesterday.")
  ) |> 
  tab_footnote(
  ...
  tab_source_note(
    source_note = "Source: pickapump.com"
  ) |> 
  gtsave("pumps.png")

# post to bluesky --------------------------------------------------------------
bs_post(
  text = glue("Average fuel prices reported on pickapump.com yesterday. #Ireland #Petrol #Diesel #FuelPrice"),
  images = "pumps.png",
  images_alt = "Table showing average fuel prices by Irish county reported yesterday on pickapump.com",

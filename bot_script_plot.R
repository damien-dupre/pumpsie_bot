...
county_title <- unique(df$county)[1]

lowest <- df |> 
  filter(price < 500 & dateupdated > (Sys.Date() - 14)) |> 
...
df |> 
  ...
  geom_jitter(aes(dateupdated, price / 100, color = fuel), alpha = 0.5, size = 0.5) +
  ...
  geom_text_repel(data = oil_df |> group_by(type) |> slice_max(Date, with_ties = FALSE), aes(Date, ((Close + 20) / 70), label = dollar(Close)), hjust = 0, direction = "y", segment.color = NA, seed = 123, size = 3, force = 0.5, family = "serif", color = "gray30") +
  geom_text_repel(data = oil_df |> group_by(type) |> slice_min(Date, with_ties = FALSE), aes(Date, ((Close + 20) / 70), label = dollar(Close)), hjust = 1, direction = "y", segment.color = NA, seed = 123, size = 3, force = 0.5, family = "serif", color = "gray30") +
  geom_text_repel(data = lowest, aes(dateupdated, price / 100, label = paste(name, addr2), color = fuel), max.overlaps = Inf) +
  scale_y_continuous(labels = dollar_format(prefix = "€")) + 
  scale_linetype_manual(values = c("twodash", "dotted")) +
  scale_color_manual(values = c("#D0BE24", "#549B8C")) +
  labs(
    title = glue("Reported prices of <span style='color:#D0BE24;'>Diesel</span> and <span style='color:#549B8C;'>Petrol</span> in Co.{county_title} in the last 14 days"),
    subtitle = "Station names correspond to the lowest prices, Brent and WTI stock prices added for information",
...
ggsave("pumps.png", width = 4, height = 4)

post_tweet(glue("Fluctuation of fuel prices reported on pumps.ie in Co. #{county_title} for the last 2 weeks. #Ireland #Petrol #Diesel #FuelPrice"), media = "pumps.png", token = bot_token)
